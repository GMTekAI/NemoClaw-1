...
The restart command sends an authenticated lifecycle request to the root-owned PID 1 supervisor through registry-scoped privileged direct-container control.
The supervisor validates `/sandbox/.hermes/.env`, the runtime environment, and the strict root-owned config hash before it relaunches the gateway under the `gateway` user.
It does not recompute that strict hash or adopt direct edits made inside the sandbox.
Use `nemohermes <name> config set` or `nemohermes inference set` for supported configuration changes so NemoClaw updates the config and trust anchors together.
PID 1 owns the Hermes gateway, dashboard, and internal API and dashboard relays; the host only checks or recovers the host-side OpenShell forwards after health passes.
...
The image must still include the generated Hermes config, NemoClaw Hermes plugin, blueprint files, `nemoclaw-start` entrypoint, root-only gateway control helper, and shared supervisor library.
...
Supervisor control requires a direct sandbox container whose PID 1 is the built-in NemoClaw entrypoint, or a custom image that preserves that entrypoint, running as root.
A VM host is supported only when its direct sandbox container uses that root entrypoint.
Non-root direct-container entrypoints, including VM deployments that launch the image as non-root, cannot use supervisor control; rebuild or recreate the sandbox with the current image contract in a supported root-entrypoint deployment.
Kubernetes and other deployments without a direct container fail closed and do not fall back to ordinary `openshell sandbox exec` or an in-sandbox manual relaunch.

...
When you change a Hermes plugin or a startup-only setting that already exists in the running sandbox, restart the Hermes gateway from the host:

```bash
nemohermes <name> gateway restart
```

The restart command keeps the gateway process under the `gateway` user, validates `/sandbox/.hermes/.env` before relaunch, and checks or recovers the Hermes API, dashboard, and declared forwards after health passes.
`recover` re-evaluates the Hermes secret boundary on every run, including healthy gateways, and repairs stopped gateways or forwards.

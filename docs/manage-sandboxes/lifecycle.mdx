...
For built-in OpenClaw and Hermes sandboxes, recovery sends an authenticated request to the root-owned PID 1 supervisor through registry-scoped privileged direct-container control.
...
The restart command asks the same PID 1 supervisor to stop its tracked gateway child, launch a replacement, and wait for health.
The host then checks or recovers host-side dashboard, messaging, and agent forwards.
For Hermes, PID 1 owns the gateway, dashboard process, and internal API and dashboard relays.
The host does not start those in-sandbox processes independently.
Refer to [`$$nemoclaw <name> gateway restart`](../reference/commands#$$nemoclaw-name-gateway-restart) for details.

The privileged lifecycle path requires a running direct sandbox container whose PID 1 is the built-in NemoClaw image entrypoint running as root.
...
Kubernetes and other deployments without a direct container also fail closed with `privileged control unavailable`.
If a direct-container sandbox uses an older image without the supervisor helper, rebuild it before retrying:

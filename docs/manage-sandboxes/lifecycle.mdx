...
<Note>
The uninstall command treats `~/.nemoclaw/rebuild-backups/` (host-side snapshots that snapshot and `backup-all` commands write), `~/.nemoclaw/backups/` (workspace backups that `scripts/backup-workspace.sh` writes), and `~/.nemoclaw/sandboxes.json` (the sandbox registry) as preserve-able user data.
...
Non-interactive runs treat `--yes` as acknowledgement of every prompt, including the data-loss prompt: `--yes` removes the preserved entries. Pass `--keep-user-data` alongside `--yes` to retain them.
On a non-TTY shell or with `NEMOCLAW_NON_INTERACTIVE=1` but no `--yes`, the preserved entries are kept by default; pass `--destroy-user-data` (or set `NEMOCLAW_UNINSTALL_DESTROY_USER_DATA=1`) to remove them.
...
The same `--yes`, `--keep-openshell`, `--delete-models`, `--destroy-user-data`, `--keep-user-data`, and `--gateway` flags listed above also apply to the hosted script. Pass them after `bash -s --`.

...
Use supported host commands such as `nemohermes <name> config set` and `nemohermes inference set` for intended runtime configuration changes because those commands update the config and its trust anchors together.
...
An older direct-container image without the supervisor helper reports `privileged control unavailable` and requires `nemohermes <name> rebuild --yes`.
...
```bash
nemohermes my-assistant rebuild [--yes|-y|--force] [--verbose|-v]
```
...
```bash
NEMOCLAW_GATEWAY_BIND_ADDRESS=0.0.0.0 NEMOCLAW_GATEWAY_PORT=8990 nemohermes onboard
```

...
Use this after a sandbox pod restart, a sandbox crash, or whenever `nemohermes <name> status` reports the gateway is not running but the sandbox is alive.
...
```bash
nemohermes uninstall [--yes] [--keep-openshell] [--delete-models] [--destroy-user-data] [--keep-user-data] [--gateway <name>]
```
...
These flags toggle optional behaviors during onboarding; set them before running `nemohermes onboard`.

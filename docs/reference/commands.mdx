...
For guidance on when to use `$$nemoclaw` versus the underlying `openshell` CLI, see [CLI Selection Guide](cli-selection-guide).
...
See [Declarative Multi-Agent Manifest](../inference/declarative-agents-manifest) for the schema and OpenClaw-native sub-agent field semantics.
...
For Hermes, restart the Hermes gateway if prompted so the removal takes effect.
...
| Flag | Effect |
|---|---|
| `--yes` | Skip all confirmation prompts and remove preserved user data; pair with `--keep-user-data` to preserve |
| `--keep-openshell` | Leave OpenShell binaries installed |
| `--delete-models` | Also remove NemoClaw-pulled Ollama models |
| `--destroy-user-data` | Remove preserved user data (`rebuild-backups/`, `backups/`, `sandboxes.json`) without requiring `--yes` |
| `--keep-user-data` | Preserve `rebuild-backups/`, `backups/`, and `sandboxes.json` under `~/.nemoclaw/` even with `--yes` |
| `--gateway <name>` | Override the gateway name to remove (default: `$$nemoclaw`) |

```bash
$$nemoclaw uninstall [--yes] [--keep-openshell] [--delete-models] [--destroy-user-data] [--keep-user-data] [--gateway <name>]
```
...
| Context | Behaviour |
|---|---|
| Interactive TTY, preserved entries present, no flag override | Prompts `Also remove them? [y/N]`. Default `N` keeps the entries. |
| Interactive TTY, user answers `y` | Removes everything under `~/.nemoclaw/`. |
| `--yes` (any context) | Acknowledges every prompt, including the data-loss prompt; removes everything under `~/.nemoclaw/`. |
| `--yes --keep-user-data` (any context) | Acknowledges the uninstall but preserves the listed entries under `~/.nemoclaw/`. |
| Non-interactive (`NEMOCLAW_NON_INTERACTIVE=1` or non-TTY shell) without `--yes` | Preserves the entries and prints a one-line notice. |
| Any context with `--destroy-user-data` | Skips the prompt and removes everything under `~/.nemoclaw/`. Takes precedence over the env var. |
| Any context with `--keep-user-data` | Skips the prompt and preserves the listed entries. Takes precedence over `--destroy-user-data`, `--yes`, and the env var. |
...
For narrative how-to coverage of `NEMOCLAW_BREV_PROVIDER` and `NEMOCLAW_GPU`, see [Deploy to Remote GPU](../deployment/deploy-to-remote-gpu).

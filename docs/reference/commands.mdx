...
- `[from <tier> tier]` — pre-selected by the chosen policy tier (see [Policy Tiers](../reference/network-policies#policy-tiers)).
- `[from openclaw agent]` or `[from hermes agent]` — added by NemoClaw to satisfy the active agent's required integrations (for example `openclaw-pricing` for OpenClaw, or Nous-hosted gateway presets for Hermes).
- `[user-added]` — applied later through `policy-add` or by including the preset during onboarding outside the tier defaults.

...
Force-restart the supported in-sandbox gateway process through the host-mediated root exec path.
...
For Hermes, the restart runs as root through `openshell sandbox exec`, validates `/sandbox/.hermes/.env` against the secret-boundary guard, validates the runtime environment, refreshes mutable config hashes only after those checks pass, and relaunches the process as the `gateway` user with `HERMES_HOME=/sandbox/.hermes`.
Locked or immutable Hermes config must match the existing strict hash.
If locked config has drifted, the command refuses instead of blessing the change.

</AgentOnly>

The command can fail at these layers: unsupported agent, root exec unavailable, secret-boundary refusal, unsafe config path, hash mismatch while locked, launch failure, or health timeout.
...
For Hermes, run `nemohermes <name> gateway restart` if prompted so the removal takes effect.

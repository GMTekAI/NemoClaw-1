...
- `[source unverified]` — the row is active but the local registry and live gateway state disagree, or the gateway is unreachable. The provenance check is suppressed in these trust-degraded states because the source cannot be confirmed against both halves of the sandbox policy view. The existing desync or unreachable messaging on the same row explains the remediation.

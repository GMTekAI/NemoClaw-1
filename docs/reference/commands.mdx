...
```bash
$$nemoclaw onboard [--non-interactive] [--resume | --fresh] [--recreate-sandbox] [--gpu | --no-gpu] [--from <Dockerfile>] [--name <sandbox>] [--sandbox-gpu | --no-sandbox-gpu] [--sandbox-gpu-device <device>] [--agent <name>] [--agents <agents.yaml>] [--control-ui-port <N>] [--yes | -y] [--no-ollama-autostart] [--yes-i-accept-third-party-software]
```
...
Refer to [Declarative Multi-Agent Manifest](../inference/declarative-agents-manifest) for the schema and OpenClaw-native sub-agent field semantics.
...
For built-in OpenClaw and Hermes sandboxes, `recover` sends an authenticated lifecycle request to the root-owned PID 1 supervisor in the sandbox.
The host reaches that supervisor through registry-scoped privileged direct-container control.
...
If the boundary validator or supervisor helper is missing, recovery fails closed and leaves an otherwise healthy gateway untouched.
Rebuild an older sandbox image with `$$nemoclaw <name> rebuild --yes` before retrying.

</AgentOnly>

The privileged control path requires a running direct sandbox container that belongs to the named registry entry and whose PID 1 is the built-in NemoClaw image entrypoint running as root.
A VM host is supported only when its direct sandbox container uses that root entrypoint.
Non-root direct-container entrypoints, including VM deployments that launch the image as non-root, cannot use this path and fail with the `privileged control unavailable` failure layer; rebuild or recreate the sandbox with the current built-in image in a supported root-entrypoint deployment.
...
Force-restart the supported in-sandbox gateway process through the root-owned PID 1 supervisor.
...
The supervisor stops only the gateway child whose process ID and process start identity match the child it tracks, launches its replacement under the gateway user, and keeps PID 1 alive if the replacement fails health checks.
For Hermes, PID 1 also owns the dashboard process and internal API and dashboard relays.
The host repairs only the host-side OpenShell forwards after the supervisor reports a healthy gateway.

<AgentOnly variant="hermes">

For Hermes, the supervisor validates `/sandbox/.hermes/.env` against the secret-boundary guard, validates the runtime environment, and verifies `/sandbox/.hermes/config.yaml` and `.env` against the root-owned strict hash before relaunching the process as the `gateway` user.
The restart never adopts direct in-sandbox edits by recomputing the trusted hash.
Use supported host commands such as `$$nemoclaw <name> config set` and `$$nemoclaw inference set` for intended runtime configuration changes because those commands update the config and its trust anchors together.
If the strict hash does not match, the command reports the `config hash mismatch` failure layer.
...
Run `$$nemoclaw <name> shields down` before a Hermes config or inference change; these commands refuse to mutate a shields-up sandbox.

</AgentOnly>

The command can fail at these layers: unsupported agent, privileged control unavailable, secret-boundary refusal, unsafe config path, config hash mismatch, launch failure, health timeout, or forward recovery failure.
An older direct-container image without the supervisor helper reports `privileged control unavailable` and requires `$$nemoclaw <name> rebuild --yes`.
...
NemoClaw also tries to stop the OpenClaw gateway inside the selected or default sandbox, which stops in-sandbox messaging channel polling for that sandbox.
...
OpenClaw-specific build-time agent configuration:

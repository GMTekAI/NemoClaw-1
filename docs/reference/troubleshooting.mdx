...
The host uses registry-scoped privileged direct-container control to send an authenticated request to the root-owned PID 1 supervisor.
It does not fall back to ordinary `openshell sandbox exec`, SSH, or a manual in-sandbox relaunch.

This control path requires a direct sandbox container whose PID 1 is the built-in NemoClaw image entrypoint running as root.
A VM host is supported only when its direct sandbox container uses that root entrypoint.
Non-root direct-container entrypoints, including VM deployments that launch the image as non-root, cannot use this path and fail with `privileged control unavailable`.
Rebuild or recreate those sandboxes with the current built-in image in a supported root-entrypoint deployment.
...
If another lifecycle request is in progress, the helper reports `SUPERVISOR_BUSY`.
This error is retryable.
Wait for the active request to finish and retry once.
If the error mentions `SUPERVISOR_REBUILD_REQUIRED`, `SUPERVISOR_UNAVAILABLE`, or a missing `nemoclaw-gateway-control` helper, the sandbox image predates the supervisor contract.
...
For a custom image, update its Dockerfile to preserve the current NemoClaw entrypoint, root-only gateway control helper, and shared supervisor library before rebuilding.
...
On Docker Desktop WSL, the patch is required for GPU passthrough.
`NEMOCLAW_DOCKER_GPU_PATCH=0` is ignored on that runtime, and onboarding logs a warning when it is set there.
...
Every Hermes restart verifies `/sandbox/.hermes/config.yaml` and `/sandbox/.hermes/.env` against the strict root-owned hash at `/etc/nemoclaw/hermes.config-hash`.
The PID 1 supervisor validates the secret boundary and runtime environment before it stops the tracked gateway, then checks the strict hash again before launch.
It does not recompute the trusted hash to adopt direct edits made inside the sandbox.
...
Hermes PID 1 remains responsible for the gateway, dashboard, and internal API and dashboard relays throughout recovery.
...
The Hermes wizard intentionally omits the Brave Search preset because Hermes does not use NemoClaw's OpenClaw web-search configuration (refer to [Quickstart with Hermes](../../hermes/get-started/quickstart) and [Network Policies](network-policies)).

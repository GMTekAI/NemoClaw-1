...
async function bestEffortCleanupSandbox(host: HostCliClient, sandboxName: string): Promise<void> {
  try {
    ...
    // Best-effort pre-cleanup mirrors the former shell test's stale sandbox removal.
  }
}
...
async function assertGatewayRecovery(host: HostCliClient, sandboxName: string): Promise<void> {
  ...
  if (!running.stdout.trim()) {
    // Preserve the former shell test's soft-skip when the shared gateway is already
  ...
  if (afterKill.stdout.trim() === "true") {
    // Preserve the former shell test's soft-skip when Docker restarts the gateway
